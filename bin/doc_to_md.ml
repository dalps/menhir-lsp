(*******************************************************************************
  ...
  Source: https://github.com/ocaml/ocaml-lsp/blob/master/ocaml-lsp-server/src/doc_to_md.ml
*******************************************************************************)
...
type t = Raw of string | Markdown of string
...
let rec inline_element_to_inline
  ...
  | { value = `Math_span text; location } ->
      ...
      Inline.Ext_math_span
        ( Inline.Math_span.make ~display:false
...
let rec nestable_block_element_to_block
  ...
  | { value = `Table ((grid, alignment), _); location } ->
      ...
      let tbl =
        let rows =
          ...
          let cell
              ((c, _) :
                Odoc_parser.Ast.nestable_block_element Odoc_parser.Ast.cell) =
  ...
  | { value = `List (kind, style, xs); location } ->
      let l =
        ...
        in
        let tight = match style with `Heavy -> false | `Light -> true in
        let type' =
          match kind with
          | `Unordered -> `Unordered '-'
          | `Ordered -> `Ordered (1, '*')
      ...
      let main_block =
        let code_block =
          let info_string =
            ...
            | Some
                { language = { value = lang; location = lang_log }; tags = _ }
              ->
...
let rec block_element_to_block
  ...
  | {
      value =
        ( `Paragraph _ | `List _ | `Modules _ | `Code_block _ | `Verbatim _
        | `Table _ | `Math_block _ );
      location = _;
    } as nestable ->
...
let translate doc : t =
  Markdown
    ...
    |> Odoc_parser.ast |> block_element_list_to_block |> Doc.make
