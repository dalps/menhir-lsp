...
let lookup_source (sourcemap : Line_directives.source_mapping list)
    ((startp, endp) as query : lexing_range) : lexing_range option =
  let open L in
  ...
  (* -1 to not count the newline in *)
  let same_col = String.length m.text - 1 = end_col in
  log "Comparing %a with mapping #%d %a " Range.pp_lexing query i
    Line_directives.pp_source_mapping m;
  log "line check: %B, column check: %B" same_line same_col;
  if same_line && same_col then (
    ...
    log "%a --> %a" Range.pp_lexing query Range.pp_lexing res;
    Some res)
  else (
    log "Failed to map source location %a" Range.pp_lexing query;
    None)
...
let hover (state : state) ~(doc : Text_document.t) ~(pos : Position.t) :
  ...
  let sym_lexrange = Range.to_lexical_positions ~doc sym_range in
  log "symbol under cursor: %a %a (%a)" (Located.pp pp_string) sym Range.pp
    sym_range Range.pp_lexing sym_lexrange;
  let+ answer = lookup_source state.sourcemap sym_lexrange in
  Hover.create
    ~contents:
      (`MarkupContent
         (MarkupContent.create ~kind:PlainText
            ~value:(spr "mapped to %a" Range.pp_lexing answer)))
...
let selection_range ({ grammar; _ } : state) ~(positions : Position.t list)
  ...
  let log s = log_src ~debug:true "mll.selection_range" s in
...
let code_actions ({ regexps; grammar; _ } : state) ~(notify_back : notify_back)
  ...
  let log s = log_src ~notify_back "mll.code_actions" s in
  ...
  log "Code action available for selection: %s %a" local_text Range.pp
    extract_range;
