...
let startofs ((startp,_):range) = startp.pos_cnum
let endofs ((_,endp):range) = endp.pos_cnum
let startline ((startp,_):range) = startp.pos_lnum
let endline ((_,endp):range) = endp.pos_lnum
let startcol ((startp,_):range) = startp.pos_cnum - startp.pos_bol + 1
let endcol ((_,endp):range) = endp.pos_cnum - endp.pos_bol

let pos_zero : position = { pos_fname = ""; pos_lnum = 0; pos_bol = 0; pos_cnum = 0 }
