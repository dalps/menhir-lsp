...
(**[producer_names producers] returns an array [names] such that
   [names.(idx) = None] if the (idx + 1)-th producer is unnamed and
   [names.(idx) = Some id] if it is called [id]. *)
...
(**[validate_pointfree_action] checks that a text fragment represents valid
   ...
   either just whitespace, or an OCaml lowercase or uppercase identifier. The
   exception [Lexpointfree.InvalidPointFreeAction] is raised if the fragment is
   invalid. [None] is returned if the text fragment is empty; [Some _] is
   returned if it is nonempty. *)

val valid_ocaml_identifier : identifier located -> bool
(**[valid_ocaml_identifier x] determines whether the string [x] is a valid OCaml
...
(**The flag [dollars] determines whether the notation [$i] in semantic actions
   is allowed or disallowed. It is mutable and can be set from the outside. This
   is dirty but convenient. A more principled approach would be to delay the
   well-formedness check for semantic actions (which is the place where this
   setting is needed) so that it takes place after parsing instead of during
   parsing. *)
