...
(* These complement [fprintf] and [output_substring], declared above. *)

let[@inline] output_char f c =
  fprintf f "%c" c

let[@inline] output_string f s =
  fprintf f "%s" s
...
let output_whitespace f (n : int) =
  assert (0 <= n);
  if String.length !whitespace < n then
    whitespace := String.make n ' ';
  output_substring f !whitespace 0 n
...
(* [rawnl] should ideally be the only place where writing a newline character
   ...
   functions, such as [locate] and [typ0] and [exprk] and [datadefcomment]. *)
...
let nl f =
  rawnl f;
  output_whitespace f !indentation

(* [indent ofs producer] increases the current indentation level by [ofs],
   prints a newline, invokes [producer], then restores the previous
   indentation level. *)
...
(* [block format] prints material between an opening keyword and a closing
   keyword, with indentation. [format] should contain just one [%a]. *)
...
(* A list of definitions, with [sep1] in front of the first definition and
   [sep2] in front of the next definitions. *)
...
(* [sharp] produces a line directive. *)

let sharp f line file =
  fprintf f "%t# %d \"%s\"%t" rawnl line file rawnl

(* [fragment f frag] prints the text fragment [frag] and optionally surrounds
   it with line number directives and with horizontal padding. Horizontal
   padding is useful in order to obtain correct column numbers in error
   messages.*)

let fragment f (frag : string located) =
  match print_line_directives with
  | Some basename ->
      let range = Located.position frag in
      (* Emit a line directive. *)
      sharp f (Range.linenum range) (Range.filename range);
      (* Emit horizontal padding. *)
      output_whitespace f (Range.column range);
      (* Print the text fragment and adjust the line counter. *)
      output_string f (Located.value frag);
      line := !line + Range.linecount range;
      (* Print a second line directive. *)
...
let parenthesized_fragment f frag =
  fragment f (Located.parenthesize frag)
...
and pat1 f = function
  ...
  | PData ("::", [ arg1; arg2 ]) ->
      (* Special case for infix cons. *)
      fprintf f "%a :: %a" pat0 arg1 pat0 arg2
...
and typ0 f = function
  | TypTextual (Declared ocamltype) ->
      parenthesized_fragment f ocamltype
  | TypTextual (Inferred t) ->
...
let typedefs f flag typedefs =
  pdefs typedef (keytyp flag) et f typedefs

(* A value definition. *)

let valdef f vd =
  match vd with
  | { valpat = PVar id; valval = EAnnot (e, ts); attribute } ->
      (* A special case where the left-hand side is a variable
         and the right-hand side carries a type annotation. *)
      fprintf f "%s%s : %a =%a" attribute id scheme ts (indent 2 expr) e
  | { valpat = p; valval = e; attribute } ->
      fprintf f "%s%a =%a" attribute pat p (indent 2 expr) e
...
let rec structure_item f item =
  ...
  | SIFunctor ([], s) ->
      structure f s
  | SIFragment fragments ->
      List.iter (fragment f) fragments
  | _ ->
    nl f;
    begin match item with
    | SIFunctor (params, s) ->
        fprintf f "module Make%a%t= %a"
          (list parenthesized_fragment nl) params
          nl
          structend s
    | SIExcDefs defs ->
        excdefs false f defs
    | SITypeDefs (flag, defs) ->
        typedefs f flag defs
...
and interface_item f item =
  ...
  | _ ->
    ...
    | IIFunctor (params, i) ->
        fprintf f "module Make%a%t: %a"
          (list parenthesized_fragment nl) params nl
          sigend i
    | IIExcDecls defs ->
        excdefs true f defs
